...
{
    /* ledG.port é o device do controlador GPIO do ledG; usamos ele para
       configurar/definir o pino OUT_PIN. */
    if (ledG.port == NULL) {
        return;
    }
    gpio_pin_set(ledG.port, OUT_PIN, value);
}
...
void force_red(void)
{
   ...
   /* Seta o pino externo em HIGH enquanto o vermelho está ativo */
    out_pin_set(1);
...
void main(void)
{
    ...
    ret = gpio_pin_configure(ledG.port, OUT_PIN, GPIO_OUTPUT_INACTIVE);
    if (ret < 0) {
        LOG_ERR("Falha ao configurar OUT_PIN (%d): %d", OUT_PIN, ret);
        return;
    }
